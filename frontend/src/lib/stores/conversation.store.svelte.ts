...
export class ConversationStore {
	...
	initializeConversation(conversationId: string, initialMessages: ConversationMessage[] = []) {
		this.#state[conversationId] = {
			isInitialized: true,
			isStreaming: false,
	...
	addUserMessage(conversationId: string, messageId: string, content: string) {
		...
		convo.messages.push({ id: messageId, role: 'user', content, status: 'sending' });
	}

	markUserMessageSent(conversationId: string, messageId: string) {
		const msg = this.#state[conversationId]?.messages.find((m) => m.id === messageId);
	...
	addAssistantPlaceholder(conversationId: string, messageId: string) {
		...
		convo.messages.push({ id: messageId, role: 'assistant', content: '', status: 'streaming' });
	}

	appendAssistantChunk(conversationId: string, messageId: string, chunk: string) {
		const msg = this.#state[conversationId]?.messages.find((m) => m.id === messageId);
		if (msg) { msg.content += chunk; msg.status = 'streaming'; }
	}
