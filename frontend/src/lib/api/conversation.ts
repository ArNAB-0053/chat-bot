...
export async function getConversations(): Promise<ConversationsPayload> {
	const response = await requestJson<ConversationsPayload>(`${env.API_URL}/conversations`, {
		method: 'GET',
	});
...
export async function getConversation(conversationId: string): Promise<ConversationPayload> {
	const response = await requestJson<ConversationPayload>(`${env.API_URL}/conversations/${conversationId}`, {
		method: 'GET',
	});
...
export async function createConversation(
	...
	const response = await requestJson<ConversationPayload>(`${env.API_URL}/conversations`, {
		method: 'POST',
		json: body,
	});
